...
def create_translated_fashion_mnist(
    root_dir="./data",
    save_dir="./translated_data",
    canvas_size=128,
    is_train=True,
):
    """
    ...
    # 2. 预分配张量
    # 图像形状：(N, 1, canvas_size, canvas_size)
    translated_images = torch.zeros(
        (num_samples, 1, canvas_size, canvas_size),
        dtype=torch.float32,
    )
    ...
    print(
        f"正在生成 {split_name}（共 {num_samples} 个样本）..."
    )
